...
const { ROOT, ELECTRON, DAEMON_ENTRY } = require('../helpers/paths')
const { connect } = require('../helpers/daemonClient')
...
async function main() {
  const daemon = spawn(ELECTRON, [DAEMON_ENTRY, `--socket=${SOCK}`, `--data-dir=${DATA}`], {
    env: {
      ...process.env,
